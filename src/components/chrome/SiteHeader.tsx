  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[var(--hairline)] bg-[color-mix(in_oklab,var(--color-ink)_82%,transparent)] backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-[1400px] items-center gap-6 px-5 sm:px-8">
        ...
        <nav aria-label={t('a11y.mainNavigation')} className="ms-auto flex items-center gap-1">
          <Link
            href={`/${locale}/destinations`}
            className="rounded-sm px-3 py-2 text-sm text-lime-dim transition-colors hover:text-lime"
          >
            {t('nav.destinations')}
          </Link>
          <Link
        ...
        <div className="border-s border-[var(--hairline)] ps-4">
